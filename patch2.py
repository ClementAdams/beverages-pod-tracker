...
# 3. Add collectionNoteNo generation (correct context this time)
old = "    if (!noteForm.manifestPhoto) { alert('Manifest photo is required'); return; }\n    setLoading(true);\n    try {\n      const fd = new FormData();"
new = "    if (!noteForm.manifestPhoto) { alert('Manifest photo is required'); return; }\n    setLoading(true);\n    try {\n      const collectionNoteNo = nextNoteSeq.toString();\n      const fd = new FormData();"
...
# 7. Add Farm Email + My Email UI fields after Accountant Email
