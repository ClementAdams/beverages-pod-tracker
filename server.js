...
const noteSchema = new mongoose.Schema({
  ...
  deliveryNoteNumber: { type: Number, unique: true }, // NEW: Sequential delivery note number
  ...
  wasteManifestPhoto: String, // NEW: Photo for waste manifest
  ...
  sendAttempts: { type: Number, default: 0 }, // NEW: Track send attempts
  lastSendError: String, // NEW: Store last error message
  sendStatus: { type: String, default: 'pending', enum: ['pending', 'sent', 'failed'] } // NEW: Send status
});
...
// NEW: Admin-only endpoint to see all user passwords (for admins who forget)
app.get('/api/admin/users-with-passwords', auth, adminOnly, async (req, res) => {
  try {
    const users = await User.find().lean();
    
    // Get all default users with their plain text passwords (they're stored in code)
    const defaultUsers = [
      { username: 'admin', password: 'admin123' },
      { username: 'lucia', password: 'Lucia@2026' },
      { username: 'clem', password: 'Clem@2026' },
      { username: 'osdam', password: 'Osdam@2026' },
      { username: 'Chill', password: 'password123' }
    ];
    
    // Map users with plain text default passwords where applicable
    const usersWithPasswords = users.map(user => {
      const defaultUser = defaultUsers.find(d => d.username === user.username);
      return {
        id: user._id,
        username: user.username,
        password: defaultUser ? defaultUser.password : '(Custom - see admin for reset)',
        role: user.role,
    ...
    res.json({ 
      ...
      note: 'Default passwords shown above. Custom password users should reset via /api/change-password endpoint'
    });
  } catch (err) { 
...
// NEW: Admin reset user password endpoint
app.post('/api/admin/reset-password/:userId', auth, adminOnly, async (req, res) => {
  try {
    ...
    res.json({ 
      success: true,
      message: `Password reset for ${user.username}. New password is: ${newPassword}`,
      username: user.username,
...
app.post('/api/collection-note', auth, upload.single('wasteManifestPhoto'), async (req, res) => {
  try {
    ...
    const { driverName, vehicleInfo, farmDestination, accountantEmail,
            driverSignature, periodStart, periodEnd, manifestNumber, collectionNoteNo, farmName } = req.body;
...
app.post('/api/collection-note/:noteId/resend', auth, async (req, res) => {
  try {
    ...
    // NEW: Update send status tracking
    note.sendAttempts = (note.sendAttempts || 0) + 1;
    if (Array.isArray(results) && results.some(r => r.sent)) {
      note.sendStatus = 'sent';
      note.lastSendError = null;
    } else {
      note.sendStatus = 'failed';
      note.lastSendError = 'Email send failed or returned errors';
    }
    
    await Note.updateOne({ noteId: req.params.noteId }, {
      sendAttempts: note.sendAttempts,
      sendStatus: note.sendStatus,
      lastSendError: note.lastSendError
    });
    
    res.json({ success: note.sendStatus === 'sent', results, attempts: note.sendAttempts });
  } catch (err) {
    // NEW: Error handling with status update
    const noteId = req.params.noteId;
    const note = await Note.findOne({ noteId }).lean();
    const attempts = (note?.sendAttempts || 0) + 1;
    
    await Note.updateOne({ noteId }, {
    ...
    res.status(500).json({ 
      error: 'Failed to send collection note',
      details: err.message,
      attempts,
      canRetry: true
    });
...
// NEW: Dedicated retry endpoint with better error handling
app.post('/api/collection-note/:noteId/retry-send', auth, async (req, res) => {
  try {
    ...
    // Update tracking
    const sendSuccess = Array.isArray(results) && results.some(r => r.sent);
    note.sendAttempts = (note.sendAttempts || 0) + 1;
    note.sendStatus = sendSuccess ? 'sent' : 'failed';
    note.lastSendError = sendSuccess ? null : 'Send failed - check logs';
    
    if (sendSuccess) {
      note.emailStatus = results.map(r => ({ 
        to: r.to, 
        sent: r.sent, 
        error: r.error || '', 
        sentAt: new Date() 
    ...
    res.json({
      success: sendSuccess,
      message: sendSuccess ? `Delivery note #${note.deliveryNoteNumber} sent successfully` : 'Send failed',
      deliveryNoteNumber: note.deliveryNoteNumber,
      attempts: note.sendAttempts,
      status: note.sendStatus,
      error: note.lastSendError
    });
  } catch (err) {
    console.error('Retry send error:', err);
    res.status(500).json({
      error: 'Retry send failed',
      details: err.message,
      canRetry: true
    });
  }
